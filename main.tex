...
\begin{document}
	...
	\newpage
	Cette seconde partie a mis en évidence une importante évolution dans la place des référentiels et les usages qui en sont faits. Ils ne se trouvent plus en marge des systèmes documentaires: les efforts entrepris dans le partage pour une réutilisation montrent un glissement vers le centre de ces systèmes, sans toutefois l'atteindre. Le lien, plus que la donnée elle-même, est devenu un enjeu pour toutes les institutions: un enrichissement de ses propres données est possible. Ces partages constants ont montré l'importance des formats et des protocoles d'échanges communs utilisés par tous.\\
	
	Deux cas de figure généraux sont apparus quant à la place des référentiels dans les institutions et aux liens qu'ils entretiennent entre eux: d'une part, il peut y avoir une simple réutilisation des données d'une institution à une autre (\reference{schema_general_relier1}); d'autre part, le Web sémantique a permis la création de nouveaux référentiels avec lesquels il est possible de créer du lien (\reference{schema_general_relier2}).\\
